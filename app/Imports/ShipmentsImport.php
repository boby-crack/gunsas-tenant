...
class ShipmentsImport extends BaseExcelImport
{
    ...
    {
        $id = (int) $this->number($row, ['id', 'shipment_id', 'pengiriman_id'], 0);
        $shipment = $id > 0 ? Shipment::find($id) : null;

        if ($id > 0 && ! $shipment) {
            throw new \InvalidArgumentException("shipment ID {$id} tidak ditemukan");
        }
        ...
        $inventoryUnitCost = $shipmentMode === 'inventory'
            ? (float) ($inventoryItem?->default_unit_cost ?? 0)
            : 0;
        $inventoryUnit = $shipmentMode === 'inventory'
            ? ($inventoryItem?->unit ?: $this->text($row, ['generic_unit', 'satuan', 'unit']))
            : null;

        return ($shipment ?? new Shipment())->fill([
